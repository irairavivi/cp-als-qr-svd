...
%CP_ALS_QR_SVD Compute a CP decomposition of any type of tensor using the QR-decomposition and SVD.
%
...
%
%   P = CP_ALS_QR_SVD(X,R) computes an estimate of the best rank-R
%   CP model of a tensor X using an alternating least-squares
%   algorithm and the QR-decomposition and SVD.  The input X can be a 
%   tensor, sptensor, ktensor, or ttensor. The result P is a ktensor.
%
%   P = CP_ALS_QR_SVD(X,R,'param',value,...) specifies optional parameters and
...
%      'svdtol' - Tolerance level for the SVD, default is 1e-8
%
...
params.addParamValue('svdtol',1e-8,@isscalar); %%% Tolerance level for the SVD. %%%
    ...
    for n = dimorder(1:end)
        ...
        if isa(Y,'ktensor')
            ...
            if implicit == true
                ...
                %%% Here we make the assumption that R is ill-conditioned. %%%
                %%% Therefore, solve for U{n} via the SVD decomposition. %%%
                %%% We can just use pinv since it uses the SVD decomposition, and set a tolerance so that %%%
                %%% the small singular values don't cause further ill-conditioning. %%%
        
                U{n} = C' * pinv(full(R)',svd_tol); 
            else
